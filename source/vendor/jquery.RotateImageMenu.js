$(function() {
		//our 4 items
	var $listItems 		= $('#gallery__container > ul > li'),
		totalItems		= $listItems.length,

		//the controls
		$gallery__next		= $('#gallery__next'),
		$gallery__prev		= $('#gallery__prev'),
		$gallery__play		= $('#gallery__play'),
		$gallery__pause		= $('#gallery__pause'),

		//the masks and corners of the slider
		$gallery__mask_left	= $('#gallery__mask_left'),
		$gallery__mask_right	= $('#gallery__mask_right'),
		$gallery__corner_left	= $('#gallery__corner_left'),
		$gallery__corner_right= $('#gallery__corner_right'),
			...
			var	timeDiff			= 200,
				...
				*/
				...
				initEventsHandler	= function() {
					...
					-1 	rotate left
					*/
					$gallery__next.bind('click', function(e) {
						stopSlideshow();
						rotateImages(1);
						return false;
					});
					$gallery__prev.bind('click', function(e) {
						stopSlideshow();
						rotateImages(-1);
						return false;
					});
					...
					*/
					$gallery__play.bind('click', function(e) {
						startSlideshow();
						return false;
					});
					$gallery__pause.bind('click', function(e) {
						stopSlideshow();
						return false;
					});
					/*
				...
				*/
				...
				rotateMaskCorners	= function() {
					$gallery__mask_left.transform({rotate: '-3deg'});
					$gallery__mask_right.transform({rotate: '3deg'});
					$gallery__corner_left.transform({rotate: '45deg'});
					$gallery__corner_right.transform({rotate: '-45deg'});
				},
				//hides the masks and corners
				hideMaskCorners		= function() {
					$gallery__mask_left.hide();
					$gallery__mask_right.hide();
					$gallery__corner_left.hide();
					$gallery__corner_right.hide();
				},
